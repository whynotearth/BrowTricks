        ...
        <BaseCard class="flex justify-center my-1">
            ...
            <template #content>
                <div class="">
                    ...
                    <br />
                    <ul class="text-left">
                        <li v-for="item in plan.features" :key="item.id"><svg class="inline ml-5" viewBox="0 -10 50 50" width="24" height="24" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" clip-rule="evenodd"><path d="M21 6.285l-11.16 12.733-6.84-6.018 1.319-1.49 5.341 4.686 9.865-11.196 1.475 1.285z"/></svg>{{ item }}</li>
                    </ul>
                </div>
            </template>

            <template #footer>
                <span class="text-center cursor-pointer" @click="viewterms = !viewterms">{{ (!viewterms) ? "VIEW" : "CLOSE"}} TERMS AND CONDITIONS</span>
                <p v-if="viewterms" class="text-left text-gray-700"><br />{{ plan.terms }}</p>
            </template>
        </BaseCard>
    </div>
</template>
